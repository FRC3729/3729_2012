...
namespace RJFRC2011
{
	...
	private:
		...
		Jaguar * _bl;
		Jaguar * _br;
		//! The last sent speed value (used for ramping, tank drive).
		float _speed_prev;
		//! The last sent turn value (used for ramping, tank drive).
		float _turn_prev;
		//! The last received x value (used for ramping, mecanum drive).
		float _x_prev;
		//! The last sent turn value (used for ramping, mecanum drive).
		float _y_prev;
	public:
		Drive();
		~Drive();
		...
		float drive_mecanum(float x, float y);
	};
};
